...
import com.example.a846205123.ncu_learningchinese.FirebaseModel.FirebaseUploadData;
...
public class ActivityCreateWord extends AppCompatActivity implements View.OnClickListener ,AdapterView.OnItemSelectedListener{
    private EditText EditEnglish,EditChinese;
    private FirebaseUploadData firebaseUploadData;
    ...
    private String PictureRename,Type;
    ...
    private void FindView(){
        ...
        EditChinese = findViewById(R.id.editChinese);
        EditEnglish = findViewById(R.id.editEnglish);
        spinnerWordType = (Spinner)findViewById(R.id.spinnerType);
        spinnerWordType.setOnItemSelectedListener(this);
        ...
        uploadImgButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!TextUtils.isEmpty(imgPath)) {
                    ...
                    Toast.makeText(ActivityCreateWord.this, R.string.plz_pick_img, Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        String text = spinnerWordType.getSelectedItem().toString();
        Type = spinnerWordType.getSelectedItem().toString().toLowerCase();
        Log.d("Type",text);

        switch (text.toLowerCase()){
            case "animal":
                firebaseUploadData =  new FirebaseUploadData(text.toLowerCase());
                break;
            case "greeting":
                firebaseUploadData =  new FirebaseUploadData(text.toLowerCase());
                break;
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btnUpload:
                if (TextUtils.isEmpty(EditEnglish.getText()) || TextUtils.isEmpty(EditChinese.getText())){
                    Toast.makeText(this,"Don't forget to enter two fields",Toast.LENGTH_LONG).show();
                    return;
                }else {
                    if(!TextUtils.isEmpty(imgPath)) {
                        mVibrator.vibrate(100);//震動
                        firebaseUploadData.UploadData(EditEnglish.getText().toString(),EditChinese.getText().toString());//上傳中文英文單字
    ...
    private void checkPermission(){
        int permission = ActivityCompat.checkSelfPermission(ActivityCreateWord.this,
    ...
    private void uploadImg(String path){
        PictureRename = EditEnglish.getText().toString()+".jpg";
        Uri file = Uri.fromFile(new File(path));
        StorageMetadata metadata = new StorageMetadata.Builder()
                .setContentDisposition("universe")
                .setContentType("image/jpg")
                .build();
        riversRef = mStorageRef.child(Type).child(PictureRename);
        ...
        if (requestCode == PICKER) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if(!TextUtils.isEmpty(imgPath)) {
                    ...
                    Glide.with(this).load(imgPath).into(pickImg);
                } else{
                    Toast.makeText(this, R.string.load_img_fail, Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
